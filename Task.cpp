...
void Task::Process() {
  ...
  qn_eventinfo_f_->SetVariable("CentralitySPD", event->CentralitySPD());
  qn_eventinfo_f_->SetVariable("VtxZ", event->Vertex(2));
  if (event_->Vertex(2) < -10 || event->Vertex(2) > 10) return;
  if (event_->CentralityVZERO() <= 0 || event->CentralityVZERO() >= 100) return;
  Qn::Interface::FillDetectors(raw_data_, *event, *histograms_);
  VAR::FillEventInfo(event, qn_manager_.GetDataContainer());

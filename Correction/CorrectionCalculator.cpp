...
/// \endcond
namespace Qn {
constexpr Int_t CorrectionCalculator::nMaxNoOfDetectors = 1024;
constexpr Int_t CorrectionCalculator::nMaxNoOfDataVariables = 11000;
const char *CorrectionCalculator::szCalibrationHistogramsKeyName = "a";
...
//  detector->AttachDetectorList(this);
}
...
const QVector *CorrectionCalculator::GetDetectorQnVectorPtr(
    const char *subdetector,
    const char *expectedstep,
    const char *altstep) const {
  (void) altstep;
  const QVector *theQnVector = nullptr;
  ...
  if (pQvecList) {
    ...
    if (TString(expectedstep).EqualTo("latest"))
      theQnVector = (QVector *) pQvecList->First();
    else
      theQnVector = (QVector *) pQvecList->FindObject(expectedstep);
  }
  return theQnVector;
}
...
/// \param altstep the name of the alternative correction step if the expected one is not found
/// \return pointer to the found Qn vector
const QVector *CorrectionCalculator::GetDetectorQnVector(
    const char *subdetector,
    const char *expectedstep,
    const char *altstep) const {
  ...
  if (pQvecList!=nullptr) {
    ...
    if (!theQnVector || !(theQnVector->IsGoodQuality()) || theQnVector->n()==0) {
      /* the Qn vector for the expected step was not there or did not have the proper quality */
      if (TString(altstep).EqualTo("latest"))
        theQnVector = (QVector *) pQvecList->First();
      else
        theQnVector = (QVector *) pQvecList->FindObject(altstep);
    }
  }
  if (theQnVector) {
    /* check the Qn vector quality */
    if (!(theQnVector->IsGoodQuality()) || theQnVector->n()==0)

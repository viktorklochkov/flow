...
/// \cond CLASSIMP
ClassImp(Qn::CorrectionQnVectorBuild);
/// \endcond
namespace Qn {
/// Default constructor
CorrectionQnVectorBuild::CorrectionQnVectorBuild() : CorrectionQnVector() {
...
CorrectionQnVectorBuild::CorrectionQnVectorBuild(const char *name, Int_t nNoOfHarmonics, Int_t *harmonicMap) :
    CorrectionQnVector(name, nNoOfHarmonics, harmonicMap) {

}

/// Copy constructor from a Q vector
/// \param Qn the Q vector build object to copy after construction
CorrectionQnVectorBuild::CorrectionQnVectorBuild(const CorrectionQnVector &Qn) :
    CorrectionQnVector(Qn) {

}

/// Copy constructor
/// \param Qn the Q vector build object to copy after construction
CorrectionQnVectorBuild::CorrectionQnVectorBuild(const CorrectionQnVectorBuild &Qn) :
    CorrectionQnVector(Qn) {

}

/// Default destructor
CorrectionQnVectorBuild::~CorrectionQnVectorBuild() {
...
void CorrectionQnVectorBuild::SetQx(Int_t, Float_t) {
...
///
/// It should not be used. Runtime error indication.
void CorrectionQnVectorBuild::SetQy(Int_t, Float_t) {
...
void CorrectionQnVectorBuild::Set(CorrectionQnVectorBuild *Qn) {

  /* the name is not copied from building Qn vectors */
  CorrectionQnVector::Set(Qn, kFALSE);
}
...
void CorrectionQnVectorBuild::Add(CorrectionQnVectorBuild *Qn) {
...
void CorrectionQnVectorBuild::NormalizeQoverM() {
...
/// A check for significant value is made. Not passing it
/// does set the Q vector quality as bad
void CorrectionQnVectorBuild::NormalizeQoverSquareRootOfM() {
...
void CorrectionQnVectorBuild::Reset() {

  CorrectionQnVector::Reset();
}

/// Print the Qn vector in a readable shape
///
void CorrectionQnVectorBuild::Print(Option_t *) const {
  cout << "OBJ: building Qn vector" << "\t" << "N: " << fN << "\t" << "Sum w: " << fSumW << "\t"
       << "quality: " << ((fGoodQuality) ? "good" : "bad") << endl;
  Int_t harmonic = GetFirstHarmonic();
  while (harmonic!=-1) {
    cout << "\t" << "\t" << "harmonic " << harmonic*fHarmonicMultiplier << "\t" << "QX: " << Qx(harmonic) << "\t"
         << "QY: " << Qy(harmonic) << endl;

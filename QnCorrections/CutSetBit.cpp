...
/// \cond CLASSIMP
ClassImp(Qn::CutSetBit);
/// \endcond
namespace Qn {

/// Default constructor
CutSetBit::CutSetBit() : CutsBase() {
  fBitMask = 0x00000000;
...
CutSetBit::CutSetBit(const CutSetBit &cut) :
    CutsBase(cut) {
...
CutSetBit::CutSetBit(Int_t varId, Int_t bitNo, Bool_t set) :
    CutsBase(varId) {
  if (nHighestBitNumberSupported < bitNo) {
    QnCorrectionsFatal(Form(
        "You requested a cut on bit %d but the highest bit number supported by the framework is currently %d",
        bitNo,
        nHighestBitNumberSupported));
  }

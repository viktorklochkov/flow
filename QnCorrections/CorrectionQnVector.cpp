...
/// \cond CLASSIMP
ClassImp(Qn::CorrectionQnVector);
/// \endcond
namespace Qn {
const Float_t  CorrectionQnVector::fMinimumSignificantValue = 1e-6;
const UInt_t   CorrectionQnVector::harmonicNumberMask[] =
    {0x0000, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
     0x0100, 0x0200, 0x0400, 0x0800, 0x1000, 0x2000, 0x4000, 0x8000};

/// Default constructor
CorrectionQnVector::CorrectionQnVector() : TNamed() {
...
Int_t CorrectionQnVector::GetNoOfHarmonics() const {
...
void CorrectionQnVector::GetHarmonicsMap(Int_t *harmonicMapStore) const {
  Int_t nNoOfHarmonics = 0;
  for (Int_t h = 1; h < fHighestHarmonic + 1; h++) {
    if ((fHarmonicMask & harmonicNumberMask[h])==harmonicNumberMask[h]) {
...
void CorrectionQnVector::Set(CorrectionQnVector *Qn, Bool_t changename) {
...
///
void CorrectionQnVector::Normalize() {
  for (Int_t h = 1; h < fHighestHarmonic + 1; h++) {
    if ((fHarmonicMask & harmonicNumberMask[h])==harmonicNumberMask[h]) {
...
Double_t CorrectionQnVector::EventPlane(Int_t harmonic) const {
...
///
void CorrectionQnVector::Print(Option_t *) const {

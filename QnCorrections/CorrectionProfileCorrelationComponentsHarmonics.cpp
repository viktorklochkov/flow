...
#include "CorrectionProfileCorrelationComponentsHarmonics.h"
#include "CorrectionLog.h"

/// \cond CLASSIMP
ClassImp(Qn::CorrectionProfileCorrelationComponentsHarmonics);
/// \endcond
namespace Qn {
/// Default constructor
CorrectionProfileCorrelationComponentsHarmonics::CorrectionProfileCorrelationComponentsHarmonics() :
...
CorrectionProfileCorrelationComponentsHarmonics::CorrectionProfileCorrelationComponentsHarmonics(const char *name,
                                                                                                       const char *title,
...
Bool_t CorrectionProfileCorrelationComponentsHarmonics::CreateCorrelationComponentsProfileHistograms(TList *histogramList,
  ...
  for (Int_t i = 0; i < nNoOfHarmonics; i++) {
    ...
    for (Int_t var = 0; var < nVariables; var++) {
      fXXValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fXXValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fXXValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fXXValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fXYValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fXYValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fXYValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fXYValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fYXValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fYXValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fYXValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fYXValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fYYValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fYYValues[currentHarmonic]->GetAxis(var)->Set(fEventClassVariables.At(var).GetNBins(),
                                                    fEventClassVariables.At(var).GetBins());
      fYYValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
      fYYValues[currentHarmonic]->GetAxis(var)->SetTitle(fEventClassVariables.At(var).GetLabel());
    }
...
Bool_t CorrectionProfileCorrelationComponentsHarmonics::AttachHistograms(TList *histogramList) {
  ...
  if (fEntries!=NULL && fEntries->GetEntries()!=0) {
    ...
    for (Int_t i = 0; i < nMaxHarmonicNumberSupported; i++) {
      ...
      fXXValues[currentHarmonic] =
          (THnF *) histogramList->FindObject(Form("%s_h%d", (const char *) histoXXName, currentHarmonic));
      fXYValues[currentHarmonic] =
          (THnF *) histogramList->FindObject(Form("%s_h%d", (const char *) histoXYName, currentHarmonic));
      fYXValues[currentHarmonic] =
          (THnF *) histogramList->FindObject(Form("%s_h%d", (const char *) histoYXName, currentHarmonic));
      fYYValues[currentHarmonic] =
          (THnF *) histogramList->FindObject(Form("%s_h%d", (const char *) histoYYName, currentHarmonic));
...
Long64_t CorrectionProfileCorrelationComponentsHarmonics::GetBin(const double *variableContainer) {
...
Bool_t CorrectionProfileCorrelationComponentsHarmonics::BinContentValidated(Long64_t bin) {
...
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetXXBinContent(Int_t harmonic, Long64_t bin) {

  /* sanity check */
  if (fXXValues[harmonic]==NULL) {
    QnCorrectionsFatal(Form("Accessing non allocated harmonic %d in correlation component histogram %s. FIX IT, PLEASE.",
                            harmonic,
                            GetName()));
    return 0.0;
  }
...
/// \param bin the interested bin number
/// \return the bin number content
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetXYBinContent(Int_t harmonic, Long64_t bin) {
...
/// \param bin the interested bin number
/// \return the bin number content
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetYXBinContent(Int_t harmonic, Long64_t bin) {
...
/// \param bin the interested bin number
/// \return the bin number content
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetYYBinContent(Int_t harmonic, Long64_t bin) {
...
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetXXBinError(Int_t harmonic, Long64_t bin) {
...
/// \param bin the interested bin number
/// \return the bin content error
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetXYBinError(Int_t harmonic, Long64_t bin) {
...
/// \param bin the interested bin number
/// \return the bin content error
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetYXBinError(Int_t harmonic, Long64_t bin) {
...
/// \param bin the interested bin number
/// \return the bin content error
Float_t CorrectionProfileCorrelationComponentsHarmonics::GetYYBinError(Int_t harmonic, Long64_t bin) {
...
void CorrectionProfileCorrelationComponentsHarmonics::FillXX(Int_t harmonic,
  ...
  if (fXXharmonicFillMask & harmonicNumberMask[harmonic]) {
    QnCorrectionsFatal(Form("Filling twice the harmonic %d before entries update in histogram %s.\n" \
        "   This means you probably have not updated the other components for this harmonic. FIX IT, PLEASE.",
                            harmonic,
                            GetName()));
  }
...
/// \param variableContainer the current variables content addressed by var Id
/// \param weight the increment in the bin content
void CorrectionProfileCorrelationComponentsHarmonics::FillXY(Int_t harmonic,
                                                                const double *variableContainer,
                                                                Float_t weight) {
  /* first the sanity checks */
  if (fXYValues[harmonic]==NULL) {
...
/// \param variableContainer the current variables content addressed by var Id
/// \param weight the increment in the bin content
void CorrectionProfileCorrelationComponentsHarmonics::FillYX(Int_t harmonic,
                                                                const double *variableContainer,
                                                                Float_t weight) {
  /* first the sanity checks */
  if (fYXValues[harmonic]==NULL) {
...
/// \param variableContainer the current variables content addressed by var Id
/// \param weight the increment in the bin content
void CorrectionProfileCorrelationComponentsHarmonics::FillYY(Int_t harmonic,
                                                                const double *variableContainer,
                                                                Float_t weight) {
  /* first the sanity checks */
  if (fYYValues[harmonic]==NULL) {

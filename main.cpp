...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<TFile> in_calibration_file(nullptr);
  std::shared_ptr<TFile> out_calibration_file(nullptr);
  std::array<std::shared_ptr<TFile>, 4> files= {in_file, out_file, in_calibration_file, out_calibration_file};
  int test[2] = {1, 1};
  Qn::Task task(files);
  task.Run();

...
/// \cond CLASSIMP
ClassImp(Qn::CutsBase);
/// \endcond
namespace Qn {
const Int_t CutsBase::nHighestBitNumberSupported = 31;

/// Default constructor
CutsBase::CutsBase() : TObject() {
  fVarId = -1;
}

/// Copy constructor
/// \param cut the cut object to clone
CutsBase::CutsBase(const CutsBase &cut) :
    TObject(cut) {
  fVarId = cut.fVarId;
}

...
#include "CorrectionProfile.h"
#include "CorrectionLog.h"

/// \cond CLASSIMP
ClassImp(Qn::CorrectionProfile);
/// \endcond
namespace Qn {
/// Default constructor
CorrectionProfile::CorrectionProfile() : CorrectionHistogramBase() {
  fValues = NULL;
  fEntries = NULL;
}
...
CorrectionProfile::CorrectionProfile(const char *name,
                                           const char *title,
...
Bool_t CorrectionProfile::CreateProfileHistograms(TList *histogramList) {
...
Bool_t CorrectionProfile::AttachHistograms(TList *histogramList) {
...
Long64_t CorrectionProfile::GetBin(const double *variableContainer) {
...
Bool_t CorrectionProfile::BinContentValidated(Long64_t bin) {
...
Float_t CorrectionProfile::GetBinContent(Long64_t bin) {
...
Float_t CorrectionProfile::GetBinError(Long64_t bin) {
  Int_t nEntries = Int_t(fEntries->GetBinContent(bin));
...
void CorrectionProfile::Fill(const double *variableContainer, Float_t weight) {

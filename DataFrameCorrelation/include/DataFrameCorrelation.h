...
class DataFrameCorrelation;

template<typename Function, typename... Qvectors, typename... InputDataContainers>
class DataFrameCorrelation<Function, std::tuple<Qvectors...>, std::tuple<InputDataContainers...>> {
 public:
  ...
  explicit DataFrameCorrelation(Function &&function) : function_(function) {}
...
template<typename F>
auto MakeDataFrameCorrelation(F lambda) {
  typedef TemplateHelpers::FunctionTraits<decltype(lambda)> traits;
  auto constexpr n_parameters = traits::Arity;
  using QVectorTuple = typename TemplateHelpers::TupleOf<n_parameters, Qn::QVector>;
  using DataContainerTuple = TemplateHelpers::TupleOf<n_parameters, Qn::DataContainerQVector>;
  return DataFrameCorrelation<F, QVectorTuple, DataContainerTuple>(std::forward<F>(lambda));
}
}
}

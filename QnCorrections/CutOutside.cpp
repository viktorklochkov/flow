...
#include "CutOutside.h"
#include "CorrectionLog.h"
/// \cond CLASSIMP
ClassImp(Qn::CutOutside);
/// \endcond
namespace Qn {
/// Default constructor
CutOutside::CutOutside() : CutsBase() {
  fMinThreshold = -1e10;
  fMaxThreshold = 1e10;
}
...
CutOutside::CutOutside(const CutOutside &cut) :
    CutsBase(cut) {
...
CutOutside::CutOutside(Int_t varId, Float_t min, Float_t max) :
    CutsBase(varId) {
  fMinThreshold = min;
  fMaxThreshold = max;
}

...
void SimpleTask::Run() {
  ...
  auto tpcptva = correlations_.at("tpcptvc").GetCorrelation();
  ...
  auto v1tpcza = tpczaxx.Apply(zazcxx.Map(sqrt),divide).Apply(tpczayy.Apply(zazcyy.Map(sqrt),divide),add).Map([](Qn::Statistics a){return a * (1/TMath::Sqrt(2));});
  auto v1tpczc = tpczcxx.Apply(zazcxx.Map(sqrt),divide).Apply(tpczcyy.Apply(zazcyy.Map(sqrt),divide),add).Map([](Qn::Statistics a){return a * (-1/TMath::Sqrt(2));});
...
void SimpleTask::Process() {
  ...
  correlations_.at("tpcptva").Fill({tpcpt, va}, eventbin, scalar);
  correlations_.at("tpcptvc").Fill({tpcpt, vc}, eventbin, scalar);

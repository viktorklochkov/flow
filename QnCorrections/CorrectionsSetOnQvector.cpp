...
/// \cond CLASSIMP
ClassImp(Qn::CorrectionsSetOnQvector);
/// \endcond
namespace Qn {
/// Default constructor
CorrectionsSetOnQvector::CorrectionsSetOnQvector() : TList() {

}

/// Default destructor
CorrectionsSetOnQvector::~CorrectionsSetOnQvector() {
...
/// its key
void CorrectionsSetOnQvector::AddCorrection(CorrectionOnQvector *correction) {
  if (IsEmpty()) {
    AddFirst(correction);
  } else if (correction->Before((CorrectionOnQvector *) First())) {
    AddFirst(correction);
  } else if (((CorrectionOnQvector *) Last())->Before(correction)) {
    AddLast(correction);
  } else {
...
void CorrectionsSetOnQvector::FillOverallCorrectionsList(TList *correctionlist) const {
    ...
    if (!correctionlist->IsEmpty()) {
      for (Int_t ix = 0; ix < GetEntries(); ix++) {
        if (correctionlist->FindObject(At(ix)->GetName())!=NULL) {
          ...
          if (At(ix)->Before((CorrectionStepBase *) correctionlist->First())) {
            correctionlist->AddFirst(At(ix));
          } else if (((CorrectionStepBase *) correctionlist->Last())->Before(At(ix))) {
            correctionlist->AddLast(At(ix));
          } else {
            for (Int_t jx = 0; jx < correctionlist->GetEntries(); jx++) {
              if (At(ix)->Before((CorrectionStepBase *) correctionlist->At(jx))) {
...
const CorrectionOnQvector *CorrectionsSetOnQvector::GetPrevious(const CorrectionOnQvector *correction) const {
...
Bool_t CorrectionsSetOnQvector::IsCorrectionStepBeingApplied(const char *step) const {
